...
## Step 1: Prepare the Dockerfile for Delve Debugging

To enable remote debugging with Delve, modify the Dockerfile to include Delve in a dedicated development stage. This configuration ensures Delve runs in headless mode, ready for remote debugging.

Replace the default Dockerfile with one that supports Delve:

```bash
mv ./Dockerfile ./Dockerfile.bkp && cp docker/delve/Dockerfile ./
```
...
## Step 4: Forward the Delve Debugger Port
...
## Step 5: Configure VS Code for Remote Debugging
