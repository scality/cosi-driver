...
func isRetryableError(err error) bool {
	...
	// Non-retryable error codes (client errors, permanent failures)
	switch code {
	case codes.OK:
		return false
	...
	case codes.InvalidArgument:
		return false // Invalid bucket name, invalid parameters, etc.
	case codes.PermissionDenied:
		return false // Access denied, insufficient permissions
	case codes.FailedPrecondition:
		return false // Bucket not empty, state conflicts
	case codes.ResourceExhausted:
		return false // Quota exceeded, limits reached
	case codes.NotFound:
		return false // Already deleted for delete operations
	...
	default:
		...
		// codes.Internal, codes.Unavailable, codes.Unknown, codes.DeadlineExceeded, codes.Aborted
		return true
	}
}
...
func (b *BucketListener) Add(ctx context.Context, inputBucket *v1alpha1.Bucket) error {
	...
	if bucket.Spec.ExistingBucketID != "" {
		...
		if err != nil {
			// FIXED: Check if error is retryable using proper error classification
			if !isRetryableError(err) {
				klog.V(3).ErrorS(err, "Non-retryable error from driver - will not retry",
					"bucket", bucket.ObjectMeta.Name,
			...
			// For retryable errors, return the error to trigger retry with backoff
...
func (b *BucketListener) deleteBucketOp(ctx context.Context, bucket *v1alpha1.Bucket) error {
	...
	if bucket.Spec.DeletionPolicy == v1alpha1.DeletionPolicyDelete {
		...
		if _, err := b.provisionerClient.DriverDeleteBucket(ctx, req); err != nil {
			// FIXED: Apply same error classification for delete operations
			if !isRetryableError(err) {
				klog.V(3).ErrorS(err, "Non-retryable error from driver for delete - will not retry",
					"bucket", bucket.ObjectMeta.Name,
					"errorCode", status.Code(err))
				return b.recordError(bucket, v1.EventTypeWarning, v1alpha1.FailedDeleteBucket, fmt.Errorf("failed to delete bucket: %w", err))
			}
			// For retryable errors, return error to trigger retry
			klog.V(3).ErrorS(err, "Retryable error from driver for delete - will retry with backoff",
				"bucket", bucket.ObjectMeta.Name,
				"errorCode", status.Code(err))
